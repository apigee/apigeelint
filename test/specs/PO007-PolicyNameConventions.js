...
const testOne = (testcase, ix, cb) => {
  const policyXml = testcase,
    doc = new Dom().parseFromString(policyXml),
    p = new Policy("", `testcase-${ix}`, this, doc);
  p.getElement = () => doc.documentElement;

  const policyName = p.getName(),
...
describe("PO007 - policy name conventions", function () {
  testCases.forEach((tc, ix0) => {
    tc.positive.forEach((name, ix1) => {
      const configXml = tc.config.replace("@@NAME@@", name);
    ...
    tc.negative.forEach((name, ix1) => {
      ...
      testOne(configXml, `${ix0} negative.${ix1}`, (p, foundIssues) => {
        ...
        assert.ok(
          p
            .getReport()
            .messages[0].message.startsWith("Non-standard name for policy"),
        );
      });
    });
  });
