...
const isPolicyOfInterest = (p) =>
  policyTypesOfInterest.find((name) => name == p.getType());
...
const onPolicy = function (policy, cb) {
  ...
  try {
    const ofInterest = isPolicyOfInterest(policy);
    if (ofInterest) {
      const expiryNodes = xpath.select(
        `/${ofInterest}/ExpirySettings`,
        policy.getElement(),
      );
      if (expiryNodes && expiryNodes.length > 0) {
        ...
        debug(
          `${policy.fileName} found ${expiryNodes.length} ExpirySettings elements`,
        );
          ...
          expiryNodes
            ...
            .forEach((node) =>
              addMessage(
                node.lineNumber,
                node.columnNumber,
                "extraneous ExpirySettings element",
              ),
            );
        }
        ...
        if (oldTimeoutNodes && oldTimeoutNodes.length > 0) {
          ...
          oldTimeoutNodes.forEach((node) =>
            addMessage(
              node.lineNumber,
              node.columnNumber,
              "TimeoutInSec is deprecated; use TimeoutInSeconds",
            ),
          );
        }
          ...
          if (timeoutNodes.length > 1) {
            timeoutNodes
              ...
              .forEach((node) =>
                addMessage(
                  node.lineNumber,
                  node.columnNumber,
                  "extraneous TimeoutInSeconds element",
                ),
              );
          } else {
            ...
            if (text) {
              ...
              if (intValue > 86400 * 30) {
                addMessage(
                  timeoutNodes[0].lineNumber,
                  timeoutNodes[0].columnNumber,
                  "value for TimeoutInSeconds is too long",
                );
              }
            }
          }
            ...
            if (text) {
              ...
              if (parts && parts.length == 3) {
                ...
                if (yyyy.length != 4 || mm.length != 2 || dd.length != 2) {
                  addMessage(
                    dateNodes[0].lineNumber,
                    dateNodes[0].columnNumber,
                    "seems like an invalid date format in ExpiryDate",
                  );
                }
                const d = new Date(
                  parseInt(yyyy, 10),
                ...
                if (isNaN(d.getTime())) {
                  addMessage(
                    dateNodes[0].lineNumber,
                    dateNodes[0].columnNumber,
                    "invalid date format in ExpiryDate",
                  );
                }
              } else {
                addMessage(
        ...
        if (foundValid == 0) {
          addMessage(
            expiryNode.lineNumber,
            expiryNode.columnNumber,
            "missing child of ExpirySettings",
          );
        }
        if (foundValid > 1) {
          addMessage(
            expiryNode.lineNumber,
            expiryNode.columnNumber,
            "multiple children of ExpirySettings",
          );
        }
      } else {
