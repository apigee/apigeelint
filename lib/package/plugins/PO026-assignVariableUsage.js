...
var cli = require('../../../cli');
var configuration = cli.configuration;
      ...
      const addMessage = (line, column, message) => {
        ...
        if (avNodes && avNodes.length>0) {
          ...
          avNodes.forEach( (node, ix) => {
            ...
            debug( "Configuration.profile is: " + configuration.profile );
            if( configuration.profile === "apigeex") {
              var found = { Name:0, Template:0, Value:0, Ref:0, PropertySetRef:0 };
            } else {
              var found = { Name:0, Template:0, Value:0, Ref:0 };
            }
