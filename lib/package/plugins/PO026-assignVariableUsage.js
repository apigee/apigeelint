...
const onPolicy = function (policy, cb) {
  ...
  if (ptype === "AssignMessage" || ptype === "RaiseFault") {
    ...
    if (avNodes && avNodes.length > 0) {
      debug(
        `${policy.getName()} found ${avNodes.length} AssignVariable elements`,
      );
      avNodes.forEach((node) => {
