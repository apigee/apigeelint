...
const plugin = {
  ...
  message:
    "TargetEndpoint HTTPTargetConnection should not use SSLInfo with LoadBalancer.",
  fatal: false,
      ...
      if (loadBalancers.length == 1) {
        if (sslInfos.length > 0) {
          endpoint.addMessage({
            ...
            message: `When using a LoadBalancer, the SSLInfo should be configured in the TargetServer, not under HTTPTargetConnection`
          });
