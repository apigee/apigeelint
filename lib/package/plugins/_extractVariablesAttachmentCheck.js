...
const messageCheckRegexp = (source) =>
  stringNonNullCheckRegexp(`${source}\.content`);
...
class EVAttachmentChecker {
  ...
  check(endpoint) {
    ...
    const reKnownMessages = "^(request|response|message)$";
    let flagged = false;
    ...
    endpoint.getSteps().forEach((step) => {
      ...
      debug(
        `found an attached ExtractVariables policy, line ${step.getElement().lineNumber}`,
      );
      const payload = xpath.select(
      ...
      if (parent) {
        if (parent.getType() === "FlowPhase") {
          debug(
            `parent type: ${parent.getType()} phase: ${parent.getPhase()}... popping up.`,
          );
      ...
      endpoint.addMessage({
        ...
        message: `For the ExtractVariables step (${policyName}), an appropriate check for a message body was not found. For example, use <Condition>${exampleCondition(source)}</Condition>`,
      });
    });
