...
let complexConditionTermCount = 12;
const onBundle = function (bundle, cb) {
  if (bundle.config.complexConditionTermCount) {
    complexConditionTermCount = bundle.config.complexConditionTermCount;
  }
...
const onCondition = function (condition, cb) {
  condition.getTruthTable().getAST(function (ast) {
    ...
    if (nodeCount > complexConditionTermCount) {
      condition.addMessage({
        ...
        plugin,
        message: `Condition contains a high number of terms, variables, constants, and comparators (n=${nodeCount}, threshold=${complexConditionTermCount}).`,
      });
    }
    if (typeof cb == "function") {
      cb(null, nodeCount);
