...
class PolicyFlowChecker {
  ...
  check(sequence) {
    let flagged = false;
    const checkerObj = this;
    try {
      const debug = this.debug;
      // const bundlePolicies = sequence.getBundle().getPolicies();
      const sequenceType = sequence.constructor.name;
...
const checkEndpoint = function (endpoint, cb) {
  let flagged = false;
  try {
    endpoint.getAllFlows().forEach((sequence) => {
