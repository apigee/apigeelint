...
const plugin = {
  ...
  message:
    "DefaultFaultRule should have only supported child elements, at most one AlwaysEnforce element, and at most one Condition element.",
  fatal: false,
...
const onDefaultFaultRule = (dfr, cb) => {
  ...
  const alwaysEnforceChildren = allChildren.filter(
    (child) => child.nodeName == "AlwaysEnforce",
  );
  if (alwaysEnforceChildren.length > 1) {
    addMessage(`Multiple AlwaysEnforce elements under DefaultFaultRule`);
  }
  const conditionChildren = allChildren.filter(
