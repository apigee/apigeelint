...
const plugin = {
  ruleId,
  name: "Check number of resources present in the bundle as a gauge of bundle size",
  message:
...
const onBundle = function (bundle, cb) {
  const limit = 20, // seems arbitrary
  ...
  if (resources.length > limit) {
    bundle.addMessage({
      plugin,
      message: `More (${resources.length}) than recommended resources (${limit}) in bundle.`,
    });
    flagged = true;
  }
