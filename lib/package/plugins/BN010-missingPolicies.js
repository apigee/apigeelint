...
// TODO: Remove this onBundle, probably unnecessary.
// let thisBundle = null;
...
const onStep = function (step, cb) {
  const source = step.getSource(),
    ...
    thisBundle = step.getBundle();
  let flagged = false;
  if (stepName) {
    /*  is not blank */
    const found = thisBundle
      .getPolicies()
      .find((policy) => policy.getName() == stepName);
    if (!found) {
      ...
      step.addMessage({
        ...
        column,
        plugin,
        message: `Missing policy "${stepName}"`,
      });
      flagged = true;
    }
  }
