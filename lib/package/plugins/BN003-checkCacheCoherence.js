...
const store = { LookupCache: {}, PopulateCache: {} };
...
const onBundle = function (bundle, cb) {
  ...
  bundle.getPolicies().forEach(function (policy) {
    if (
      ...
      // Ok now we have the key values for the cache policy; stow them if
      // they don't already exist. Use a composite key.
  ...
  for (const lookupKey in store["LookupCache"]) {
