...
// TODO: evaluate where symbols are added, and record them in our symbol table.
const evaluateSteps = function (steps, localSymtab) {
  steps.forEach(function (step) {
    if (step.getName()) {
      analyzeVariables(step.getName(), localSymtab).forEach(function (badvar) {
        const result = {
          ruleId: plugin.ruleId,
          ...
          nodeType: plugin.nodeType,
          message: `Variable {${badvar}} was used in step name, but not previously defined.`,
        };
    ...
    if (step.getCondition()) {
      analyzeVariables(step.getCondition().getExpression()).forEach(
        function (badvar) {
          const result = {
            ruleId: plugin.ruleId,
            severity: plugin.severity,
            ...
            column: step.getElement().columnNumber,
            nodeType: plugin.nodeType,
            message: `Variable {${badvar}} was used in step condition, but not previously defined.`,
          };
          step.addMessage(result);
          hadWarnErr = true;
        },
    ...
    const policy = glBundle.getPolicyByName(step.getName());
    policy &&
...
const analyzeVariables = function (value, localSymtab) {
  const symtab = globalSymtab.concat(localSymtab);
  const undefinedVars = [];
  let match;
...
const _identity = function (policy, localSymtab) {};
const handlers = {
  // TODO: lots more handlers!
  ExtractVariables(policy, localSymtab) {
    const payloadVariablesXpath =
      "/ExtractVariables/*[self::JSONPayload or self::XMLPayload]/Variable/@name";
    policy
      .select(payloadVariablesXpath)
      .forEach((variableDecl) => localSymtab.push(variableDecl.value));
  },
