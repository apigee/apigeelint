...
const plugin = {
  ...
  message: "There should be no VirtualHost in a ProxyEndpoint for ApigeeX.",
  fatal: false,
  severity: 1, //1=warn
...
const onProxyEndpoint = function (ep, cb) {
  if (ep.getType() == "ProxyEndpoint") {
    // exclude SharedFlows
    const profile = ep.getBundle().profile,
      connections = ep.select("/ProxyEndpoint/HTTPProxyConnection");
    let flagged = false;
    ...
    if (connections.length) {
      if (connections.length > 1) {
        ...
        ep.select("/ProxyEndpoint/HTTPProxyConnection/VirtualHost").forEach(
          (vhost) => {
            ep.addMessage({
              plugin,
