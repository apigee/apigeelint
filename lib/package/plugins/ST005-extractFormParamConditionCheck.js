...
const stringNonNullCheckRegexp = (source) =>
  `\\(? *\\b${source}\\b *(!=|NotEquals|IsNot) *(\"\"|null) *\\)?`;

const messageCheckRegexp = (source) =>
  stringNonNullCheckRegexp(`${source}.formstring`);
const paramCheckRegexp = (source, name) =>
  stringNonNullCheckRegexp(`${source}.formparam.${name}`);

const exampleFormstringCondition = (source, _params) =>
  `${source}.formstring != null`;
const exampleParamCondition = (source, params) =>
  params.map((name) => `${source}.formparam.${name} != null`).join(" || ");

const isSufficient = function (condition, source, paramNames) {
  const exp = condition.getExpression();
  debug(`isSufficient cond(${exp})`);
  const retval =
    exp.match(messageCheckRegexp(source)) ||
    paramNames.some((name) => exp.match(paramCheckRegexp(source, name)));
  debug(`> isSufficient() ${!!retval}`);
  return retval;
};

const check = function (endpoint) {
  let flagged = false;
  const bundlePolicies = endpoint.parent.getPolicies();

  endpoint.getSteps().forEach((step) => {
    const referredPolicy = bundlePolicies.find((p) =>
      p.getSteps().find((s) => s == step),
    );
    if (!referredPolicy || referredPolicy.getType() !== "ExtractVariables") {
      return;
    }
    debug("found an attached ExtractVariables policy");
    const formParam = xpath.select(
    ...
    const paramNames = xpath
      .select(`/ExtractVariables/FormParam/@name`, referredPolicy.getElement())
      .map((attr) => attr.value);

    debug(`param names: (${paramNames})`);
    const sourceElement = xpath.select(
    ...
    flagged = true;
    const policyName = xpath.select1(
