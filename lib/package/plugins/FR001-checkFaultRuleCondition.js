...
const ruleId = require("../myUtil.js").getRuleId(),
      ...
      plugin = {
        ...
        message:
        "With multiple FaultRules, all except one should have a condition statement. With a single FaultRule and no Condition, consider migrating to DefaultFaultRule.",
        fatal: false,
        severity: 2, //2=error
        nodeType: "FaultRule",
        enabled: true
          ...
          rulesToCheck.forEach(fr => {
            if (!fr.getCondition() ||
              ...
              fr.addMessage({
                ...
                message: `More than one FaultRule, and FaultRule (${name}) has no Condition or the Condition is empty.`
              });
              this.flagged = true;
            }
          });
        }
        else if (allFaultRules.length == 1) {
