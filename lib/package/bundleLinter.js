...
var fs = require("fs"),
  ...
  BundleSharedFlow = require("./BundleSharedFlow.js"),
...
var lint = function(config, done) {
  if (config.source.path.includes('sharedflowbundle')){
  //if(config.type === 'sharedflows'){
    new BundleSharedFlow(config, function(bundle, err) {
      if (err) {
        if (done) {
          done(null, err);
        } else {
      ...
      bundle.excluded = config.excluded;
        ...
        fs.readdirSync(normalizedPath).forEach(function(file) {
          if (!config.plugins || contains(config.plugins, file)) {
            try {
              if(file !== "checkBundleStructure.js"){
    ...
    if (err) {
      ...
      fs.readdirSync(normalizedPath).forEach(function(file) {
        if (!config.plugins || contains(config.plugins, file)) {
          try {
            if(file !== "checkSharedFlowBundleStructure.js"){
            executePlugin(file, bundle);
            }
          } catch (e) {
            debug("plugin error: " + file + " " + e);
