...
function getSource(obj) {
  //reads the snip of text that is source for the current element
  var fName = getFileName(obj),
    startLine = obj.line,
    endLine = element.nextSibling.line,
    curLine = 0,
    result;
  require("fs")
    .readFileSync(fname)
    .toString()
    .split("\n")
    .forSome(function(line) {
      if (curLine >= startLine && curLine <= endLine) {
        result += line;
      }
      curLine++;
      //this sucks - read the file only once, cursor to where you want to start, etc.... FIXME
    });
  return result;
}
