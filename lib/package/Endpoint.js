...
class Endpoint extends ConfigElement {
  ...
  #getFlow(expr) {
    debug(`> getFlow[${expr}]()`);
    if (expr in this.#valueTable) {
      debug(`< getFlow[${expr}]() cached`);
    } else {
      const n = xpath.select(expr, this.element);
      this.#valueTable[expr] = n && n[0] && new Flow(n[0], this);
      debug(`< getFlow[${expr}]()`);
    }
    return this.#valueTable[expr]; // maybe null or false
  }
  ...
  getFlows() {
    ...
    if (!this.flows) {
      ...
      const nodes = xpath.select("/*/Flows", this.element),
        ep = this,
        flows = [];
      if (nodes) {
        nodes.forEach(function (flowsElement) {
          ...
          xpath.select("Flow", flowsElement).forEach((elt) => {
            ...
            const flow = new Flow(elt, ep);
            debug(`getFlows() flow= ${flow.element.tagName}`);
            flows.push(flow);
          });
        });
      }
  ...
  onSteps(pluginFunction, callback) {
    const endpoint = this,
      ...
      sharedflow = endpoint.getSharedFlow();
    debug(`> onSteps: endpoint name: '${endpoint.getName()}'`);
    try {
      if (sharedflow) {
        sharedflow.onSteps(pluginFunction, getcb(`onSteps sharedflow `));
      } else {
        if (endpoint.getPreFlow()) {
          endpoint
            .getPreFlow()
            .onSteps(pluginFunction, getcb("onSteps preflow"));
        }
  ...
  onConditions(pluginFunction, callback) {
    ...
    try {
      if (sharedflow) {
        sharedflow.onConditions(
          pluginFunction,
          getcb(`onConditions sharedflow`),
        );
      } else {
        if (endpoint.getPreFlow()) {
          endpoint
        ...
        if (endpoint.getPostClientFlow()) {
          endpoint
            .getPostClientFlow()
            .onConditions(pluginFunction, getcb("onConditions postClientFlow"));
        } else {
          debug("onConditions: no PostClientFlow");
        }
        if (endpoint.getDefaultFaultRule()) {
          endpoint
            .getDefaultFaultRule()
