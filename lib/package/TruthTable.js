  ...
  if (!this.evaluation) {
    if (this.expression !== "") {
      ...
      var tree = this.getAST(),
        ...
        combinations = generateCombinations(vars.length + cons.length),
...
TruthTable.prototype.summarize = function() {
  ...
  summary.validity = this.evaluation;
  return summary;
};
...
function interpret(tree, substitutions) {
  ...
  var actions = {
    substitution(args) {
      ...
      if (args[0].type === "variable") {
        ...
        //do a type conversion here
        //but really belongs in the node creation logic so not repeated
      }
      return result;
    },
    negation(args) {
  ...
  function process(node) {
    ...
    /*if (action && action.value === "!") {
      action = "negation";
    } else if (action && action.value === "equivalence") {
      action = "equivalence";
    }*/
...
function _evaluate(evaluation, tree, vars) {
  ...
  var result =
    interpret(tree, initializator.apply(initializator, evaluation)) || true;
  return result;
}
