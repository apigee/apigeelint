  ...
  if (!this.evaluation) {
    if (this.expression !== "") {
      ...
      for (let i = 0; i < c2.length; i++) {
        ...
        const result = interpret(tree, c2[i]);
      ...
      if (a[i].args) {
        ...
        if (a[i].args[0].action && a[i].args[0].action === "substitution") {
          ...
          if (
            ...
            const vName = a[i].args[0].args[0].value;
            const vVal = a[i].args[1].args[0].value;
            if (!vals[vName]) {
              vals[vName] = [];
            }
            ...
            const vName = a[i].args[1].args[0].value;
            const vVal = a[i].args[0].args[0].value;
            if (!vals[vName]) {
              vals[vName] = [];
            }
            //add check to see if already contains
...
function interpret(tree, substitutions) {
  if (!tree) return;
  const iTree = JSON.parse(JSON.stringify(tree)),
    evaluations = [];
...
TruthTable.prototype.getTokens = function () {
  ...
  function encodeQuotedChars(source, find, replace) {
    ...
    let result = _encodeWithin(source, find, replace, '"');
  ...
  if (!this.tokens) {
    const tokens = [];
    let expression = " " + this.expression; //in case the expression starts with a logical operator
    ...
    const reNotEq = new RegExp(" (Not|!) +(Equals|=|==) +", "gi");

    if (reNotEq.test(expression)) {
      const m = reNotEq.matches(expression);
      throw new Error(
    ...
    function processHangLine(tokenSegments) {
      ...
      if (tokenSegments.length > 1) {
        ...
        if (maxIndex) {
          ...
          const cNode = node(tokenSegments[maxIndex].value, []),
      ...
      if (tokenSegments[0].type === "variable") {
        ...
        const tl = tokenSegments[0].value.toLowerCase();
        if (tl === "true" || tl === "false") {
          ...
          try {
            ...
            // not a valid token
            debug(
              `Exception (${e}), not a valid token: (${tokenSegments[0].value})`,
            );
...
function generateCombinationsFromVars(nodes, vars) {
  ...
  const cartesianProduct = (arr) =>
    arr.reduce(
      (a, c) => a.flatMap((accItem) => c.map((cItem) => [...accItem, cItem])),
      [[]],
    );
