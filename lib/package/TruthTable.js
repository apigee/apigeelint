...
function TruthTable(expression, parent) {
...
function interpret(tree, substitutions) {
  ...
  var actions = {
    ...
    like(args) {
      var result = {
        action: "equivalence",
        value: args[0].evaluation.value.toLowerCase(args[1].evaluation.value.toLowerCase())>=0
      };
      return result;
    },
    notLike(args) {
      var result = {
        action: "equivalence",
        value: args[0].evaluation.value.toLowerCase(args[1].evaluation.value.toLowerCase())==-1
      };
      return result;    }
  };
...
TruthTable.prototype.getTokens = function() {
  ...
  if (!this.tokens) {
    ...
    expression = replaceAll(expression, " Not Like ", " !~ ");
    expression = replaceAll(expression, " !Like ", " !~ ");
