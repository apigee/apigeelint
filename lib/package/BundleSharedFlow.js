...
var FindFolder = require("node-find-folder"),
  ...
  Policy = require("./PolicySharedFlow.js"),
  Endpoint = require("./SharedFlowEndpoint.js"),
  xpath = require("xpath"),
...
Bundle.prototype.getName = function() {
  if (!this.name) {
    ...
    try {
      var attr = xpath.select("/SharedFlowBundle/@name", this.getElement());
...
Bundle.prototype.getRevision = function() {
  if (!this.revision) {
    //look at the root of the proxy for the .xml file and get name from there
    try {
      var attr = xpath.select("/SharedFlowBundle/@revision", this.getElement());
...
function processFileSystem(config, bundle, cb) {
  ...
  bundle.report = {
    filePath: bundle.root.substring(bundle.root.indexOf("/sharedflowbundle")),
    errorCount: 0,
    warningCount: 0,
    fixableErrorCount: 0,
  ...
  if (!bundle.proxyRoot.endsWith("sharedflowbundle")) {
    var folders = new FindFolder("sharedflowbundle");
    //potentially have sharedflowbundle 
    //we prefer the target bundle when we have both
    //refactor to contains then walk through options
    if (folders.includes("sharedflowbundle")) {
      bundle.proxyRoot = "sharedflowbundle";
      buildAll(bundle);
    } else {
      //warn and be done
      bundle.addMessage({
        message: "No sharedflowbundle folder found in: " + JSON.stringify(folders)
      });
    }
  }
  if (cb) {
...
function Bundle(config, cb) {
  ...
  if (config.source.type === "ManagementServer") {
    ...
    var fileStream = fs.createWriteStream(tempFolder + "/sharedflowbundle.zip");
        ...
        onRes: function(res) {
          if (res.statusCode === 200) {
            res.pipe(fileStream).on("close", function() {
              decompress(
                tempFolder + "/sharedflowbundle.zip",
                tempFolder
              ).then(files => {
                //add the info to the config object here
                config.source.path = tempFolder + "/sharedflowbundle";
...
Bundle.prototype.getSharedFlowEndpoints = function() {
  //if (!this.proxyEndpoints) {
		buildsharedflows(this);
    if (!this.proxyEndpoints) {
      this.proxyEndpoints = [];
...
Bundle.prototype.getEndpoints = function() {
 // if (!this.endpoints) {
    this.endpoints = this.getSharedFlowEndpoints();
