...
var fs = require("fs"),
  ...
  debug = require("debug")("bundlelinter:PolicySharedFlow");
...
function Policy(path, fn, parent) {
  ...
  this.parent = parent;
  this.report = {
    filePath: this.filePath.substring(this.filePath.indexOf("/sharedflowbundle")),
    errorCount: 0,
    warningCount: 0,
    fixableErrorCount: 0,
...
Policy.prototype.getDisplayName = function() {
  if (!this.displayName) {
    ...
    if (doc && doc[0] && doc[0].childNodes && doc[0].childNodes[0]) {
      ...
      //look for a name attribute on the policy
      var attr = xpath.select("//@name", this.getElement());
      this.displayName = (attr[0] && attr[0].value) || "";
