...
function processFileSystem(config, bundle, cb) {
  ...
  let files = fs.readdirSync(bundle.root)
    .filter( file => fs.lstatSync(path.join(bundle.root, file)).isFile() && file.endsWith('.xml') );
  ...
  let bundleTypeName = config.source.bundleType;
  ...
  if (!bundle.proxyRoot.endsWith(bundleTypeName)) {
    let folders = findFolders(bundleTypeName);
    //potentially have (apiproxy and target/apiproxy) or shared flow
    if (bundleTypeName == bundleType.BundleType.APIPROXY && folders.includes("target/apiproxy")) {
      ...
      //warn and be done
      bundle.addMessage({
        message: "No " + bundleTypeName + " folder found in: " + JSON.stringify(folders)
      });
    }
  }
  if (cb) {
...
Bundle.prototype.getDefaultFaultRules = function() {
  if (!this.defaultFaultRules) {
    ...
    this.getEndpoints().forEach(function(ep) {
          defaultFaultRules.push(ep.getDefaultFaultRule());
    });
