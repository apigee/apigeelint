...
const fs = require("node:fs"),
  ...
  domOptions = lintUtil.domOptions(debug);
      ...
      fs.readdirSync(folder).forEach(function (endptFile) {
        ...
        if (endptFile.endsWith(".xml") && fs.lstatSync(fqFname).isFile()) {
          const doc = xpath.select(
            tag,
            new Dom(domOptions).parseFromString(
              fs.readFileSync(fqFname).toString(),
            ),
          );
...
function buildProxyEndpoints(bundle) {
  const folder = path.join(bundle.proxyRoot, "proxies"),
    tag = "ProxyEndpoint",
...
function buildsharedflows(bundle) {
  const folder = path.join(bundle.proxyRoot, "sharedflows"),
    tag = "SharedFlow",
    processFunction = function (element, fqFname, bundle) {
      bundle.proxyEndpoints = bundle.proxyEndpoints || [];
...
function buildTargetEndpoints(bundle) {
  const folder = path.join(bundle.proxyRoot, "targets"),
    tag = "TargetEndpoint",
...
function buildResources(bundle) {
  ...
  bundle.resources = [];
  if (fs.existsSync(path.join(bundle.proxyRoot, "resources"))) {
    _buildResources(
      bundle,
      path.join(bundle.proxyRoot, "resources"),
      bundle.resources,
    );
  }
}
...
function processFileSystem(config, bundle, cb) {
  ...
  if (files.length > 1) {
    debug("More than one .xml file found at bundle root. Aborting.");
    throw new Error(
      "There is more than one .xml file found at the bundle root",
    );
  }
  ...
  if (!bundle.proxyRoot.endsWith(bundleTypeName)) {
    bundle.addMessage({
      message: `No ${bundleTypeName} folder found in: ${bundle.root}`,
    });
  }
...
class Bundle {
  ...
  getElement() {
    ...
    if (!this.element && fs.existsSync(filePath)) {
      ...
      this.element = new Dom(domOptions).parseFromString(
  ...
  getRevision() {
    if (!this.revision) {
      ...
      try {
        ...
        debug(`While getting revision: ${e}`);
      ...
      result.forEach((item) => {
        if (item.messages && item.messages.length) {
          ...
          if (
            ...
            this.#isFile(item.filePath)
          ) {
            try {
              const docElement = new Dom(domOptions).parseFromString(
  ...
  addMessage(msg) {
    ...
    if (
      !msg.source &&
      msg.entity &&
      typeof msg.entity.getSource == "function"
    ) {
  ...
  getPolicyByName(pname) {
    return this.getPolicies().find((policy) => policy.getName() === pname);
  }

  onBundle(pluginFunction, cb) {
    pluginFunction(this, cb);
  }

  onPolicies(pluginFunction, cb) {
  ...
  onFaultRules(pluginFunction, cb) {
  ...
  onDefaultFaultRules(pluginFunction, cb) {
  ...
  onProxyEndpoints(pluginFunction, cb) {
  ...
  onTargetEndpoints(pluginFunction, cb) {
