...
class Flow extends ConfigElement {
  constructor(element, parent) {
    ...
    this.condition = null;
    this.flowResponse = undefined;
    this.flowRequest = undefined;
  ...
  getFlowResponse() {
    ...
    let self = this;
    if (this.flowResponse == undefined) {
  ...
  onConditions(pluginFunction, callback) {
    ...
    if (flow.getSharedFlow()) {
      flow
        .getSharedFlow()
        .onConditions(pluginFunction, getcb(`onConditions Sharedflow`));
    } else {
      if (flow.getFlowRequest()) {
        flow
          .getFlowRequest()
  ...
  summarize() {
    ...
    if (this.getSharedFlow()) {
      summary.steps = this.getSharedFlow().getSteps();
    } else {
      summary = {
        ...summary,
        condition:
