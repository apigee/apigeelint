...
primary
  ...
  / ws* "(" ws* t1:token ws+ op_not ws+ op1:op_accepts_variable_and_negation ws+ t2:token ws* ")" ws* {
  ...
  / ws* "(" ws* t1:token ws+ op_not ws+ op1:op_accepts_variable_and_negation_sym ws* t2:token ws* ")" ws* {
    let operand = { operator:op1, operands:[t1, t2]};
    return {operator:"NOT", operands:[operand] };
  }
...
value
  = '"' value:ZeroOrMoreNotQuote '"' { value.unshift("'"); value.push("'"); return value; }
  /  "null" { return null; }
  / "false" { return false; }
  / "true" { return true; }
